...
class Stripe {
    ...
    public function create_card($customer_id, $token) {
        $params = array(
            "source" => $token
		);
        return $this->_send_request( 'customers/'.$customer_id.'/sources', $params );
    }

    public function update_customer_source($customer_id, $card_id) {
        $params = array(
	 ...
	 */
	public function customer_create( $email, $desc = NULL, $plan = NULL ) {
		$params = array(
			'email' => $email
		);
		if( $desc )
    ...
    public function update_plan( $subscription_item_id, $plan ) {
		$options['plan'] = $this->PLANS[$plan];
		return $this->_send_request( 'subscription_items/'.$subscription_item_id, $options, STRIPE_METHOD_POST );
    }
    ...
    public function retrieve_source($customer_id, $card_id) {
    ...
    public function get_subscription_item($subscription_item_id) {
		return $this->_send_request( 'subscription_items/'.$subscription_item_id );
    }
